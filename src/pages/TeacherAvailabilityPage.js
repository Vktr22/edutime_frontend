            ...
            .catch((err) => {
                alert(
                    err.response?.data?.message ||
                        "Nem sikerült törölni az idősávot.",
                );
            });
    }
    ...
    function saveSlot(day) {
        ...
        createAvailability(token, payload)
            ...
            .catch((err) => {
                alert(
                    err.response?.data?.message ||
                        "Nem sikerült menteni az idősávot.",
                );
            });
    }
            ...
            <div className="week-grid">
                {days.map((day, index) => (
                    <div
                        key={index}
                        className={
                            "day-column" + (isPastDay(day) ? " is-past" : "")
                        }
                    >
                        ...
                        {availability
                            .filter(
                                (slot) => slot.date === toLocalYYYYMMDD(day),
                            )
                            .map((slot) => (
